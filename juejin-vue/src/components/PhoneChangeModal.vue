...
const canSendCode = computed(() => countdown.value === 0 && !isSending.value)
const canSubmit = computed(() => verificationCode.value.length === 6)
    ...
    <Transition name="fade">
      <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-lg shadow-xl w-96 p-6">
          ...
          <!-- 标题 -->
          <h3 class="text-lg font-bold text-text-primary mb-4">手机换绑</h3>
          ...
          <!-- 下一步按钮 -->
          <button
            class="w-full h-10 bg-brand text-white text-sm font-medium rounded-lg hover:bg-brand-dark transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
            :disabled="!canSubmit"
            @click="handleSubmit"
          >
            下一步
